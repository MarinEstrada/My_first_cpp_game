...
float a_movement = 0.1f; //currently this number is units per frame... need to make it units per second

static void simulate_game(Input* input) {
		//render_background();
		clear_screen(0x00009f);
		if (pressed(BUTTON_UP)) x_player_pos += a_movement;
		if (pressed(BUTTON_DOWN)) x_player_pos -= a_movement;
		if (pressed(BUTTON_RIGHT)) y_player_pos += a_movement;
		if (pressed(BUTTON_LEFT)) y_player_pos -= a_movement;
